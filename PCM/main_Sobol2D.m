...
% INPUT:
% ----------------------------------------------------------------------------------------------------------------------
numRandomVariables=2; % do not modify
numPoints1=11; % number of quadrature points per random dimension

probabilityDistribution='Beta'; % options: Uniform, Beta or Normal
% ----------------------------------------------------------------------------------------------------------------------
...
X1=zeros(numPoints1,numPoints1);
X2=zeros(numPoints1,numPoints1);
...
W1=zeros(numPoints1,1);
W2=zeros(1,numPoints1);
...
W12=zeros(numPoints1,numPoints1);
...
% ----------------------------------------------------------------------------------------------------------------------
G0=zeros(numPoints1,numPoints1);

G0(:,:)=sum(sum(G.*W12)); % total mean

G1=zeros(numPoints1,numPoints1);
G2=zeros(numPoints1,numPoints1);

for i1=1:numPoints1
    G1(i1,:)=sum(G(i1,:).*W2)-G0(i1,:);
end

for i2=1:numPoints1
    G2(:,i2)=sum(G(:,i2).*W1)-G0(:,i2);
end

G12=zeros(numPoints1,numPoints1);
...
% ----------------------------------------------------------------------------------------------------------------------
DG=sum(sum(G.^2.*W12))-G0(1,1)^2; % total variance

D1=sum(G1(:,1).^2.*W1);
D2=sum(G2(1,:).^2.*W2);

D12=sum(sum(G12.^2.*W12));
% ----------------------------------------------------------------------------------------------------------------------
